...
async function startServer() {
  ...
  app.post("/api/create-class", async (req, res) => {
    ...
    try {
      ...
      const existingRolls = new Set(existingStudents?.map(s => s.enrollment_number) || []);
      ...
      for (const roll of newRolls) {
        ...
        if (authError) {
          if (authError.message.includes('already been registered')) {
             // Handle orphaned auth user case gracefully
  ...
  app.post("/api/provision-student", async (req, res) => {
    ...
    try {
      if (!/^\d{4}[A-Za-z]{2}\d{5,6}$/.test(enrollmentNumber)) {
        ...
        if (existingStudent) {
          ...
          if (authError) {
            if (authError.message.includes('already been registered')) {
              ...
              if (!existing) {
                 throw new Error("Email already registered, but fetching the user ID failed. Please contact admin to clean up the orphaned auth account.");
              }
